...
then
  ...
  values=$( /nodetool/dsc-cassandra-3.0.9/bin/nodetool -h ${CASSANDRA_URL} -u cassandra -pw ${JMX_PASSWORD} cfstats smartmeter  | grep "Local write count" | rev | cut -d ' ' -f 1 | rev )
