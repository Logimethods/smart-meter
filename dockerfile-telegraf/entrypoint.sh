...
if [ -z "$NODE_ID" ]
then
    export DOCKER_TARGET_ID=$(wget -q -O - http://eureka:5000/container/name/${DOCKER_TARGET_NAME})
else
    export DOCKER_TARGET_ID=$(wget -q -O - http://eureka:5000/service/name/${NODE_ID}/${DOCKER_TARGET_NAME})
fi
