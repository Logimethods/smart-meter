...
then
  ...
  values=$( /nodetool/dsc-cassandra-3.0.9/bin/nodetool -h ${CASSANDRA_LOCAL_URL} -u cassandra -pw ${JMX_PASSWORD} cfstats ${TELEGRAF_CASSANDRA_TABLE} | grep "$TELEGRAF_CASSANDRA_GREP" | rev | cut -d ' ' -f 1 | rev )
