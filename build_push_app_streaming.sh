...
docker  -H localhost:2374  service create     --name prediction_oracle     -e NATS_URI=nats://smartmeter:xyz1234@nats:4222     -e SPARK_MASTER_URL=local[*]     -e HDFS_URL=hdfs://hadoop:9000     -e CASSANDRA_URL=cassandra_cluster_main     -e STREAMING_DURATION=5000     -e LOG_LEVEL=INFO     -e SPARK_CORES_MAX=1     -e ALERT_THRESHOLD=116     --network smartmeter     --mode global     --constraint=node.role!=manager     logimethods/smart-meter:app-streaming  "com.logimethods.nats.connector.spark.app.SparkPredictionOracle"       "smartmeter.voltage.raw.forecast.12" "smartmeter.voltage.extract.prediction.12"       "Smartmeter PREDICTION ORACLE"
