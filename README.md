...
* First, create your `docker-compose-merge.yml` file thanks to [combine_services.sh](./compose/combine_services.sh), which makes use of [yamlreader](https://github.com/ImmobilienScout24/yamlreader):
    * When Docker Secrets are provided:    
    ...
    or `docker run logimethods/smart-meter:app_compose-1.0 "_secrets" root_metrics inject_metrics streaming_metrics prediction_metrics > docker-compose-merge.yml`
    * When Docker Secrets are NOT provided:    
    ...
    or `docker run logimethods/smart-meter:app_compose-1.0 "_no-secrets" root_metrics inject_metrics streaming_metrics prediction_metrics > docker-compose-merge.yml`
